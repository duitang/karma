...
public class MemoryServer {
	...
	final static String[] PARAMETER_KEYS = {
			"server",
			"client",
			"port",
			"host",
	...
	static void runServer(Map<String, String> param) {
		...
		if (param.containsKey("verbose")) {
			...
			Logger logger = (Logger) LoggerFactory.getLogger("org.apache.commons.pool2.impl.GenericObjectPool");
...
class LoadRunner implements Runnable {
	...
	public LoadRunner(
			...
			int loop,
			String msg,
			ClientFactory<IDemoService> fac,
			boolean usemap,
			int trace) {
		this.latch = latch;
		this.loop = loop;
	...
	public void run() {
		...
		try {
			...
			System.out
					.println(name + " running elapsed: " + ts + "ms with loop=[" + loop + "] @" + i + ", error=" + err);
