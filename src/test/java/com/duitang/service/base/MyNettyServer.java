...
public class MyNettyServer implements Server {
	...
	public MyNettyServer(Responder responder, InetSocketAddress addr) {
		this(responder, addr, new NioServerSocketChannelFactory(Executors.newCachedThreadPool(),
		        Executors.newCachedThreadPool()));
	}

	public MyNettyServer(Responder responder, InetSocketAddress addr, ChannelFactory channelFactory) {
		this(responder, addr, channelFactory, null);
	}

	/**
	 * @param executionHandler
	 *            if not null, will be inserted into the Netty pipeline. Use
	 *            this when your responder does long, non-cpu bound processing
	 *            (see Netty's ExecutionHandler javadoc).
	 * @param pipelineFactory
	 *            Avro-related handlers will be added on top of what this
	 *            factory creates
	 */
	public MyNettyServer(Responder responder, InetSocketAddress addr, ChannelFactory channelFactory,
